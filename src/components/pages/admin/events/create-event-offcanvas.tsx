...
// ============ CREATE EVENT MODAL ============
interface CreateEventOffcanvasProps {
    isOpen: boolean
    onClose: () => void
    onEventCreated?: () => void
}
...
export function CreateEventOffcanvas({ isOpen, onClose, onEventCreated }: CreateEventOffcanvasProps) {
    const [loading, setLoading] = useState(false)
    const [isAnimating, setIsAnimating] = useState(false)
    const [bannerFile, setBannerFile] = useState<File | null>(null)
    const [bannerPreview, setBannerPreview] = useState<string>("")
    const [eventImagesFiles, setEventImagesFiles] = useState<File[]>([])
    const [eventImagesPreviews, setEventImagesPreviews] = useState<string[]>([])
    ...
    const removeEventImage = (index: number) => {
        ...
        setEventImagesPreviews(prev => prev.filter((_, i) => i !== index))
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await axiosInstance.post("/admin/events", submitFormData, {
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder="e.g., March 15, 2025 or 15-03-2025"
                                        />
                                    </div>
                                        ...
                                        <div className="flex items-center justify-center w-full">
                                            ...
                                            >
                                                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                                    <FiUpload className="w-8 h-8 mb-4 text-gray-500" />
                                                    <p className="mb-2 text-sm text-gray-500">
                                                        <span className="font-semibold">Click to upload event images</span>
                                                    </p>
