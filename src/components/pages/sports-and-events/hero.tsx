...
const Announcements = () => {
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        ...
        const formattedAnnouncements: Announcement[] = fetchedAnnouncements.map((item: any, index: number) => ({
          id: item._id || index + 1,
  ...
  // Auto-play functionality - only start when announcements are loaded
  useEffect(() => {
  ...
  // Reset current slide when announcements change
  useEffect(() => {
  ...
  if (loading) {
    return (
      <section className="py-20 relative bg-[url(/assets/images/bg-2.webp)] overflow-hidden w-full bg-center bg-no-repeat bg-cover">
        <div className="max-w-screen-xl mx-auto px-4 flex items-center justify-center min-h-[400px]">
        ...
        <div className="max-w-screen-xl mx-auto px-4 flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <button 
              ...
              className="bg-cardinal-pink-400 text-white px-6 py-2 rounded-lg hover:bg-cardinal-pink-500 transition-colors"
            >
              Try Again
            </button>
          </div>
  ...
  return (
    <section className="py-20 relative bg-[url(/assets/images/bg-2.webp)] overflow-hidden w-full bg-center bg-no-repeat bg-cover">
      <div className="max-w-screen-xl mx-auto grid lg:grid-cols-2 px-4 lg:mb-5">
              ...
              {announcements.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    index === currentSlide 
                      ? 'bg-cardinal-pink-400 scale-110' 
        ...
        {/* Enhanced Rotating Carousel Section */}
                ...
                >
                  <div
                    className={`relative overflow-hidden rounded-2xl transition-all duration-300 ${
                      isActive ? "shadow-2xl shadow-black/30" : "shadow-lg shadow-black/20"
                    }`}
                    style={{
